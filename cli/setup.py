...
def setup_neovim(args):
    ...
    source_path = os.path.join(root, "adapters", "neovim", "leeter.lua")
    if not os.path.exists(source_path):
        print(f"[leeter] Error: Cannot find {source_path}")
        return

    with open(source_path, "r") as f:
        lua = f.read()

    # The source file uses dynamic path resolution based on its location in `adapters/neovim/leeter.lua`.
    # When we copy it, we hardcode the `bin_` to ensure it works reliably regardless of where it's loaded from.
    lua = lua.replace(
        ...
        f'local bin = opts.bin or "{bin_}"'
    )

    if args.scope == "global":
        out_path = os.path.expanduser("~/.config/nvim/lua/leeter.lua")
        print("[leeter] Add  require('leeter').setup()  to your init.lua to activate.")
    else:
        out_path = os.path.join(root, ".nvim.lua")
        # For local project scope, we need to automatically call setup() at the end
        lua += "\n-- Auto-execute setup for project-local .nvim.lua\nM.setup()\n"
...
def setup_emacs(args):
    ...
    # The emacs script adds "python3 " dynamically if the path ends with .py
    bin_path = bin_.replace("python3 ", "")

    source_path = os.path.join(root, "adapters", "emacs", "leeter.el")
    if not os.path.exists(source_path):
        print(f"[leeter] Error: Cannot find {source_path}")
        return
    ...
    if args.scope == "global":
        ...
        # Hardcode the bin path since it's no longer in the repo
        el = re.sub(
            r'\(or \(executable-find "leeter"\).*?default-directory\)\)\)\)',
            f'"{bin_path}"',
            el,
            flags=re.DOTALL
        )
